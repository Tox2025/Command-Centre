// Futures Paper Trading Client JS
...
// ── Stats ────────────────────────────────────────────
function fetchStats() {
    fetch('/api/futures-paper/stats?version=' + activeVersion).then(function (r) { return r.json(); }).then(function (s) {
        ...
        if (s.longStats && s.shortStats) {
            ...
            h += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">';
            h += '<span style="color:#f87171;font-weight:600">📉 SHORT</span>';
        ...
        if (s.bestTrade) {
            q += '<div style="display:flex;justify-content:space-between;margin-bottom:4px;font-size:10px"><span>Best</span><span style="color:var(--bull)">' + s.bestTrade.ticker + ' ' + s.bestTrade.strategy.toUpperCase() + ' +$' + s.bestTrade.pnl.toFixed(2) + '</span></div>';
        }
        if (s.worstTrade) {
            q += '<div style="display:flex;justify-content:space-between;font-size:10px"><span>Worst</span><span style="color:var(--bear)">' + s.worstTrade.ticker + ' ' + s.worstTrade.strategy.toUpperCase() + ' $' + s.worstTrade.pnl.toFixed(2) + '</span></div>';
        }
...
function fetchOpenPositions() {
    fetch('/api/futures-paper/trades?version=' + activeVersion).then(function (r) { return r.json(); }).then(function (trades) {
        var open = trades.filter(function (t) { return t.status === 'OPEN'; });
        $('openCount').textContent = open.length + ' position' + (open.length !== 1 ? 's' : '');

        if (open.length === 0) {
            $('openPositions').innerHTML = '<div class="pt-empty"><div class="pt-empty-icon">📋</div>No open futures trades.<br>Use manual entry above.</div>';
            return;
        }

        var h = '<div class="pt-table-wrap"><table class="pt-table"><thead><tr>';
        h += '<th>Ticker</th><th>Type</th><th>Entry Pts</th><th>Current Pts</th><th>Unrlzd P&L</th><th>Lots</th><th>Conf</th><th>Opened</th><th>Session</th><th>Actions</th>';
        h += '</tr></thead><tbody>';

        open.forEach(function (t) {
            var pnlClass = t.unrealizedPnl >= 0 ? 'pnl-positive' : 'pnl-negative';
            var typeClass = t.strategy === 'long' ? 'badge-call' : 'badge-put';
            var openedStr = '--';
            if (t.openTime) {
            ...
            h += '<td><span class="' + typeClass + '">' + t.strategy.toUpperCase() + '</span></td>';
            h += '<td>' + (t.entryPoints || 0).toFixed(2) + '</td>';
            h += '<td>' + (t.currentPoints || 0).toFixed(2) + '</td>';
...
function fetchTradeHistory() {
    fetch('/api/futures-paper/trades?version=' + activeVersion).then(function (r) { return r.json(); }).then(function (trades) {
        ...
        if (closed.length === 0) {
            $('tradeHistory').innerHTML = '<div class="pt-empty"><div class="pt-empty-icon">📊</div>No closed futures trades yet.<br>Close an open position to see P&L results here.</div>';
            return;
        }

        var h = '<div class="pt-table-wrap"><table class="pt-table"><thead><tr>';
        h += '<th>Ticker</th><th>Type</th><th>Lots</th><th>Entry Pts</th><th>Exit Pts</th><th>P&L</th><th>Outcome</th><th>Conf</th><th>Horizon</th><th>Entry Time</th><th>Exit Time</th><th>Duration</th>';
        h += '</tr></thead><tbody>';
        ...
        closed.forEach(function (t) {
            ...
            h += '<td>' + t.contracts + '</td>';
            h += '<td>' + (t.entryPoints || 0).toFixed(2) + '</td>';
            h += '<td>' + (t.exitPoints || 0).toFixed(2) + '</td>';
...
function manualEntry() {
    var ticker = ($('qeTicker').value || '').toUpperCase().trim();
    var strategy = $('qeType').value;
    var proxyPrice = parseFloat($('qeProxyPrice').value) || 0;
    var contracts = parseInt($('qeContracts').value) || 1;

    if (!ticker || !proxyPrice) {
        alert('Fill in ticker and proxy price');
        return;
    }

    fetch('/api/futures-paper/open', {
        method: 'POST',
        ...
        body: JSON.stringify({
            ...
            contracts: contracts,
            signalVersion: activeVersion || 'vF1'
        })
    })
        .then(function (r) { return r.json(); })
        .then(function (data) {
...
function closeTrade(id) {
    if (!confirm('Close this futures trade at current market price?')) return;
    fetch('/api/futures-paper/close', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
